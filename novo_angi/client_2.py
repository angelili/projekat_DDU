    ...
    def fit(self, parameters, config):
        ...
        for r in range(10):
            ...
            for batch_idx, (data, target) in enumerate(self.trainloader):
                optimizer.zero_grad()
                objective, loss, output, target = objective_function(local_model, self.model, 100, data.to(DEVICE), target.to(DEVICE))
            ...
            with torch.no_grad():
              for param, global_param in zip(local_model.parameters(), self.model.parameters()):
                  global_param.data=global_param.data-0.005*100*(global_param.data-param.data)
