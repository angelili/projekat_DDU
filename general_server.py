...
#The following metrics are the same the difference is in keys from the dictionaries we are filling in
...
def get_evaluate_fn_fedavg(
    ...
    def evaluate(
        ...
        dict_acc["accuracy_centralized"].append(accuracy)
        dict_loss["loss_centralized"].append(loss)
        # return statistics
        return loss, {"accuracy": accuracy}
...
def weighted_average_fedavg(dict: Dict) -> Metrics:
    ...
    def evaluate (metrics: List[Tuple[int, Metrics]]) -> Metrics:
        accuracies = [num_examples * m["accuracy"] for num_examples, m in metrics]
        examples = [num_examples for num_examples, _ in metrics]
        dict["accuracy_global"].append(sum(accuracies)/sum(examples))
        # Aggregate and return custom metric (weighted average)
        return {"accuracy_global": sum(accuracies) / sum(examples)}
    return evaluate
